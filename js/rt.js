...
async function addSoloScore(doc) {
  // Write to Firestore when available; otherwise persist locally.
  if (fbReady()) {
    const { firestore } = fb;
    ...
    try {
      ...
      arr.push({ ...doc, createdAt: Date.now() });
      localStorage.setItem('ct_solo_scores', JSON.stringify(arr));
    } catch (e) {
      console.warn('Failed to store solo score locally', e);
    }
  }
}
async function getSoloTop(limitN=20) {
  // Fallback to local scores when Firebase isn’t ready.
  if (!fbReady()) {
    try {
      const arr = JSON.parse(localStorage.getItem('ct_solo_scores') || '[]');
      arr.sort((a,b) => {
...
async function clearSoloScores() {
  ...
  try {
    localStorage.removeItem('ct_solo_scores');
  } catch {}
}
